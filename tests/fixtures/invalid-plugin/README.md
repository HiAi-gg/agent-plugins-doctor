...
```
Failed to load plugin: plugin.json does not conform to plugin.schema.json (2 violations)
```

### Why exit 3 and not a list of DOC-xxxx diagnostics

The vendored plugin.schema.json enforces the same constraints that the rules
package would otherwise check, and manifest validation is **fatal**:
...
- `plugin.json` failures are fatal by design (§11.3): the loader stops, so the
  other components are never inspected. For reference, if the manifest loaded:
...
Use this fixture to test the **load-failure path** (exit 3, "Failed to load
plugin" message). Use `warning-plugin` for the DOC-1004 warning path.
